  ...
  return (
    <div className="relative flex flex-col h-full w-full overflow-hidden">
      ...
      <div
        className={
          messages.length === 0
            ? 'flex-1 overflow-auto pt-16 flex justify-center items-center px-4' // 메시지가 없을 때
            : 'flex-1 basis-[90%] overflow-auto px-4' // 메시지가 있을 떄
        }
      >
        {messages.length === 0 ? (
      ...
      <div
        className={
          messages.length === 0
            ? 'flex-1 flex justify-center items-top px-4' // 메시지가 있을 때 basis-[20%] 제거
            : 'flex-1 flex justify-center items-center basis-[10%] px-4' // 메시지가 없을 때 basis-[20%] 추가
        }
      >
        <div className="w-full max-w-2xl">
          <form onSubmit={handleSendMessage} className="flex w-full space-x-2">
            <input
              ...
              className="flex-1 px-4 text-white py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent min-w-0"
            />
            <button
              type="submit"
